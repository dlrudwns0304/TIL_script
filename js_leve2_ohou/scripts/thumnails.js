...
// 콘솔 확인 시 1개의 DOM요소가 아닌 [index]표시된 여러개 DOM요소로 출력될때는 실제 사용시 요소 [index]를 붙여서 사용해야 한다.
...
function bigThumSrcBorder(num,target) {
    ...
    // 현재 마우스 올린 작은 썸변수에 테두리 생성(this는 현재이벤트대상 활용)
    target.style.border = '3px solid #0aa5ff'
    return bigThum.src = `./images/big${num}.jpg`
}
//this
// * 특정 DOM여소 이벤트(click mouseover등 ..) 내에서 작성하는 키워드
// 이벤트 요소가 달라도 this는 이벤트 내에서 해당 요소를 가리키는 통합명칭으로 사용할 수 있기 때문에 쉬운 선언이 가능하다
// 주의점1) 화살표함수는 사용 x 익명함수 o
// DOM.이벤트리스너(이벤트, ()=>{this}) x
// DOM.이벤트리스너(이벤트, function()=>{this}) o
// 주의점2) 이벤트 내 호출하는 함수라 해도 함수 내부에 this를 직접적으로 작성해선 안되며 이벤트 익명함수 내에서 작성해야 한다 
