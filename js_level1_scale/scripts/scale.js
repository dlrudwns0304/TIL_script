//scale.js
//다이어트 체중계 알고리즘
// 사용자가 본인 키,몸무게 입력하고 버튼을 클릭하면 함수가 실행
// 계산식) 적정체중 = (본인신장-100)*0.9
// 결과예) 당신의 적정체중은 ?kg이며 ?kg 초과되셨습니다.
// 미만, 적정체중은 조건문 배워야 하므로 나중에 추가예정
// 객체 이벤트 함수 순서 진행
...
// 사용자가 키를 입력한 후 프로그램이 그값을 인식하게 만드는법
// 값을 실시간으로 입렿하는 동안은 프로그램 과부하 및 보안 분제로 함수를 작동하게 해선 안되며 결정버튼을 따로 만들어두고 버튼에 특정동작을 해야만 함수가 실행되게 만들어야 한다.
...
function weightCal() {
    ...
    // 사용자몸무게 - 적정체중 = 초과kg
    // 최종결과예 ) 당신의 적정체중은 ?kg이며 ?kg 초과되셧습니다
    console.log(`당신의 적정제중은 ${normalWeight}kg 이며 ${userWeight.valuez   -normalWeight}kg 초과되셨습니다. `)
    // input 요소일 경우 값 읽기/수정/삭제는 value 속성 사용
    // input이 아닌 다른요소일경우 값 읽기/수정/삭제 textContent 또는 innerHTML 속성 사용
    result.innerHTML= (`당신의 적정제중은 ${normalWeight}kg 이며 ${userWeight.value   -normalWeight}kg이며 <br> <em>초과</em>되셨습니다. `)
}
