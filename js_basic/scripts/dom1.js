...
// 클릭 했을 때 - 이벤트
// 회원만 메세지 출력 - 함수
// 이벤트 리스너 -> 리스너란 특정 이벤트가 발생했을 때 함수를 의미
...
// 아이디중복확인 클릭 시 '아이디가 중복되지 않습니다. 사용하셔도 좋습니다'.
...
// pw 입력창 객체 준비 후 키보드를 아무키나 입력했을 때 '빔리번호는 특수문자 포함 12글자 이상 입력하세요' 출력 알고리즘
...
console.log(userPw);
function pwCharaCters() {
    return console.log('12글자 이상 입력하세요');
}
// 기타이벤트
// focus 특정 요소에 포커스(커서 활성화) 됐을 경우
//userId.style.border = '3px solid green'; // 즉시실행테스트
userId.style.ouline = 'none';
function focusGreen() {
...
// 쇼핑몰 상품 수량 입력 알고리즘
// 기본수량 1
// 사용자가 숫자 입력을 위해서 커서를 두면 배경색 노랑색
// 커서를 빼면 배경색 흰색
...
// addEventListener 이벤트 작성시 'on' 없이 이벤트만 작성
...
function kakaoGo () {
    return window.open('https://www.kakao.com' , '_blank');
}
// 현재 페이지 변경방법
// 1. 객체
...
indexLink.addEventListener('click', firstIndex);
function firstIndex () {
    return window.location.href = 'index.html'
}
const readmeLink =  document.querySelector('.readme_link');
console.log(readmeLink);
readmeLink.addEventListener('click', readmeMd);
function readmeMd () {
...
function numPlus() {
    // 수량이 1씩 증가한다
    // 기존의 input 값이  항상 1증가해야한다
    // 1. 증감연산자 활용
    // return numInput.value = ++numInput.value;
    // 2. 더하기연산자와 형변환 메서드 활용법
