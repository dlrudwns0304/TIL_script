...
// 버튼으로 배경색변경제어 알고리즘(마우스 올렸을 때)
bg1Btn.addEventListener('mouseover', bgBtnOver);
function bgBtnOver () {
    return bg.style.backgroundColor = 'aqua';
}
bg1Btn.addEventListener('mouseout', baBtnOut);
function baBtnOut () {
    return bg.style.backgroundColor ='yellow'; 
}
//bg2Btn.addEventListener('mouseover', bgChange('coral'));
// (위 에러 해결법) 객체 이벤트에서 매개변수가 있는 함수를 호출하려면 익명함수 또는 화살표함수로 변경해야 가능하다
