...
//--------------------예제2
let x = 5;
//let y = ++x;   y는 1증가된 x값을 대입받는 y변수
let y = x+1 // 1증가된 x값을 대입받는 y변수 일회성 더하기 연산자로 +1을 부여서 계산하고 x에 영향을 주지않게 y에 대입한다
console.log(x,y); // 5 6
y = x++
console.log(x,y); // 6 5
x = ++y
...
console.log('------------------------------');
let number = 10;
// number = 15; // 기존값을 제거하고 단순 새로운값 삽입 x
// number = 5; //  기존값을 제거하고 단순 새로운값 삽입 x
// number = number + 5; // 기존값을 유지하고 원하는 계산 추가 0
number +=5; //복합대입 활용(위와 결과동일)
